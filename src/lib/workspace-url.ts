...
export function workspaceParams(path:string, search:string) {
  ...
  if(parts[1] === "workspace") {
    ...
    params.set(["access","profile","appearance","notifications","shortcuts"].includes(page) ? "view" : "module",page);
  }
  return params;
}
