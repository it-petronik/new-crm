...
import { Plus, ArrowDownLeft, ArrowUpRight, Wallet, CalendarDays } from "lucide-react";
...
export function Cashbook({ records, actor, company, onAdd, onOpen }: { records: RecordItem[]; actor: Actor; company: string; onAdd: () => void; onOpen: (r: RecordItem) => void }) {
  ...
  const pagination = usePagination(entries, `${company}|${currency}|${type}`);
  ...
  return <section className="panel cashbook-panel">
    ...
    <ListFilters {...pagination} label="entries" sortable={false}/>
    {pagination.total > 0 && <div className="table-scroll"><table><thead><tr>{([["name","Description"],["company","Company"],["attributes","Type / Category"],["due","Transaction date"],["amount","Amount"],["owner","Created by"],["status","Status"]] as const).map(([key,label]) => key === "attributes" ? <th key={key}>{label}</th> : <SortHeader key={key} sortKey={key} query={pagination.query} setQuery={pagination.setQuery}>{label}</SortHeader>)}</tr></thead><tbody>{pagination.items.map(r => <tr key={r.id}><td><Button className="record-link avatar-name" onClick={() => onOpen(r)}><Avatar name={r.title} size={32}/><span>{r.title}<small>{r.attributes?.reference || r.contact}</small></span></Button></td><td>{companyName(r.company)}</td><td>{r.attributes?.entryType}<small>{r.attributes?.category} · {r.attributes?.department}</small></td><td><span className="cell-date"><CalendarDays size={14} aria-hidden="true"/>{r.due}</span></td><td>{money(r.amount,r.currency)}</td><td>{r.owner ? <span className="avatar-name"><Avatar name={r.owner} size={26}/><span>{r.owner}</span></span> : "—"}</td><td>{r.status}</td></tr>)}</tbody></table></div>}
