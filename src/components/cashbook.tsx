...
export function Cashbook({ records, actor, company, onAdd, onOpen }: { records: RecordItem[]; actor: Actor; company: string; onAdd: () => void; onOpen: (r: RecordItem) => void }) {
  ...
  return <section className="panel cashbook-panel">
    ...
    {pagination.total > 0 && <div className="table-scroll"><table><thead><tr>{([["name","Description"],["company","Company"],["attributes","Type / Category"],["due","Transaction date"],["amount","Amount"],["owner","Created by"],["status","Status"]] as const).map(([key,label]) => key === "attributes" ? <th key={key}>{label}</th> : <SortHeader key={key} sortKey={key} query={pagination.query} setQuery={pagination.setQuery}>{label}</SortHeader>)}</tr></thead><tbody>{pagination.items.map(r => <tr key={r.id}><td><Button className="record-link" onClick={() => onOpen(r)}>{r.title}<small>{r.attributes?.reference || r.contact}</small></Button></td><td>{companyName(r.company)}</td><td>{r.attributes?.entryType}<small>{r.attributes?.category} · {r.attributes?.department}</small></td><td>{r.due}</td><td>{money(r.amount,r.currency)}</td><td>{r.owner || "—"}</td><td>{r.status}</td></tr>)}</tbody></table></div>}
